...
class Nbaddons_Notices{
    ...
    public function _init() {        
        ...
        add_action( 'wp_ajax_baddonsnotice-ajax', [ $this, '__dismiss' ] );
             ...
             if( in_array($screen->id , $support) || $k == 'intro_pages' || $k == $key){
					self::push(
						[
							...
							'dismissible' => true,
							'return'	  => $data,
							'message'     => esc_html__($data['des'], 'next-addons' ),
						]
					);
              }
    ...
    public function get_edd_api(){
        return Nbaddons_Init::instance()->get_edd_api();
    }
	...
	public function __enqueue_scripts() {
		echo "
			...
			jQuery(document).ready(function ($) {
				$( '.naddonspro-notice.is-dismissible' ).on( 'click', '.notice-dismiss', function() {
					
					_this 		= $( this ).parents( '.naddons-pro-active' );
	...
	public static function push($notice) {
		...
		$classes = [ 'naddonspro-notice', 'notice' ];
	...
	public static function markup( $notice = [] ) {
		...
		<div id="<?php echo esc_attr( $notice['id'] ); ?>" class="naddonsnotices <?php echo esc_attr( $notice['classes'] ); ?>" <?php _e( $notice['data'] ); ?> style="background-image: url(<?php echo esc_url($img_url);?>); <?php _e( $styles );?>" >
